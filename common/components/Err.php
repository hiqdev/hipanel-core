...
class Err {
    static  public function is ($data,$eq=null) {
        return is_null($eq)
            ? ( is_array($data) ? array_key_exists('_error',$data) : !$data )
            : ( isset($data['_error']) && $data['_error']==$eq )
            ;
    }
    ...
    static  public function set ($data,$code,$ops=null) {
        if (!is_array($data)) $data = array('_error'=>$code);
    ...
    static  public function reduce ($res) {
        ...
        if ($err_num) $res['_error'] = $row_num<2
            ? reset($errors)
            : (($row_num>$err_num) ? 'partially' : 'completely')." failed ($err_num/$row_num): ".re::cjoin(array_unique($errors))
        ;
        /* if ($set_nums) {
